...
class roomeleven:
    ...
    def room(self):
        clear()
        global monster
        monster = monsterbattle("Extreme Wizard",200,70,self.health,self.sword_damage)
        room_run = True
        playerin = ""
        while room_run:
            clear()
            playerin = inputhandler.check_input_room("eleven")
            clear()
            if playerin == "w":
                self.forwardpart()
            elif playerin == "d":
    ...
    def forwardpart(self):
        ...
        if self.item in self.inventory:
            ...
            if player_answer == "yes":
                clear()
                self.sword_damage += (self.sword_damage *.1)
                print(f"You picked up the {self.item} and it contained burning spell, you apply the spell to your sword and it increased your sword damage by 10 percent\nyour sword damage is now {self.sword_damage}\n")
                self.room_number = 11
