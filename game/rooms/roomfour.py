...
class roomfour:
    def __init__(self, player_name,player_health,player_total_health,player_inventory,sword_damage):
        ...
        self.item = "body armour"
        self.got_item = False
    def room(self):
        clear()
        global monster
        monster = monsterbattle("The Toe Tickler",140,15,self.health,self.sword_damage)
        room_run = True
        playerin = ""
        while room_run:
            clear()
            playerin = inputhandler.check_input_room("four")
            clear()
            if playerin == "w":
                self.forwardpart()
            elif playerin == "d":
    ...
    def rightpart(self):
        ...
        if self.item in self.inventory:
            ...
            if player_answer == "yes":
                clear()
                health_percent_to_add = int(self.total_health * .20)
                self.total_health += health_percent_to_add
                self.inventory.append(self.item)
                print(f"You picked up {self.item} increasing your maximum health to {health_percent_to_add}\n")
                self.room_number = 4
