...
class roomfive:
    def __init__(self, player_name,player_health,player_total_health,player_inventory,sword_damage):
        ...
        self.item = "leg armour"
        self.got_item = False
    def room(self):
        clear()
        global monster
        monster = monsterbattle("Demogorgon",250,10,self.health,self.sword_damage)
        room_run = True
        playerin = ""
        while room_run:
            clear()
            playerin = inputhandler.check_input_room("five")
            clear()
            if playerin == "w":
                self.forwardpart()
            elif playerin == "d":
    ...
    def leftpart(self):
        ...
        if self.item in self.inventory:
            ...
            if player_answer == "yes":
                clear()
                health_percent_to_add = int(self.total_health * .15)
